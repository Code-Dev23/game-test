...
public class Arena {
    ...
    public void removePlayer(Player player) {
        ...
        if(state == GameState.COUNTDOWN && players.size() < ConfigManager.getRequiredPlayers()) {
            sendMessage(Format.color("&cNon ci sono abbastanza player. In attesa di altri player..."));
            reset(false);
            return;
        }

        if(state == GameState.LIVE && players.size() < ConfigManager.getRequiredPlayers()) {
            sendMessage(Format.color("&cIl game è finito perchè troppi player sono usciti."));
            reset(false);
        }
    }
